...
function generateImage() {
  ...
  fetch("/get_item_data", {
    ...
    .then((data) => {
      ...
      if (data.image_url) {
        handleData(data, itemImage, itemInputImage);
      } else if (itemInputImage) {
        itemImage.src = itemInputImage;
      } else {
        itemErrorDisplay.innerHTML =
          "Failed to generate image. Please enter your own image link.";
...
function handleData(data, itemImage, itemInputImage) {
  ...
  const selectedQuality = document.querySelector(
    '.radio-option-quality input[type="radio"]:checked'
  ).value;
  const titleBox = document.getElementById("title-box");

  itemBox.className = `item-box border-${selectedQuality}`;
  itemImage.src = itemInputImage ? itemInputImage : data.image_url;
  // show particle image if particle_id is present
  particleImage.style.display = data.particle_id ? "block" : "none";
  particleImage.src = data.particle_id
    ? `static/tf2folio/particles/${data.particle_id}_188x188.png`
    : "";
