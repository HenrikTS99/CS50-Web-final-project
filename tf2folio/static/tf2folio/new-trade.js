...
const TRANSACTION_IMAGES = {
    [TRANSACTION_METHODS.KEYS]: 'https://steamcdn-a.akamaihd.net/apps/440/icons/key.be0a5e2cda3a039132c35b67319829d785e50352.png',
    [TRANSACTION_METHODS.SCM_FUNDS]: 'https://community.cloudflare.steamstatic.com/public/shared/images/responsive/share_steam_logo.png',
    [TRANSACTION_METHODS.PAYPAL]: 'https://developer.valvesoftware.com/w/images/thumb/f/f9/Smallcredits.png/300px-Smallcredits.png'
};
...
function toggleForm() {
    ...
    document.querySelector('.items-received-selection').style.display = isSale ? 'block' : 'none';
    document.querySelector('#sold-selection-title').textContent = isSale ? 'Add sold item' : 'Add bought item';
...
function appendNewItem(data) {
    itemsContainer = isReceivedItemForm ? receivedItemsContainer : selectedItemsContainer;
...
function handleCashTransactionMethod(transactionMethod) {
    ...
    const defaultCurrency = transactionMethod == TRANSACTION_METHODS.PAYPAL ? defaultPaypalCurrency : defaultSCMCurrency;
...
function handleItemsTransactionMethod(transactionMethod) {
    ...
    const keysRadioButton2 = [...purchaseTransactionRadioButtons].find(radioButton2 => radioButton2.value == TRANSACTION_METHODS.KEYS);
    if (keysRadioButton2) {
...
function setTransactionBoxDisplay(transactionMethod, quality) {
    ...
    transactionSpan.textContent = transactionMethod == TRANSACTION_METHODS.SCM_FUNDS ? 'scm funds' : transactionMethod;
}
...
function updateTransactionBoxCurrency(transactionMethod) {
    ...
    if (currencyField.value !== '' && transactionMethod !== TRANSACTION_METHODS.KEYS) {
        boxSpan.textContent += ` ${currencyField.value}`;
    }
}
